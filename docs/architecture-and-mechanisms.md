...
| `policy.rs` | **Command policy table**: 130 commands registered across eight gate tiers + three completeness tests (below) |
...
### 7.13 Skin entrance/exit (fade in/out)

Skin windows appear/leave with a fade (in: 180 ms / out: 150 ms, both ease-out) driven by **page-content opacity** (CSS opacity on `documentElement`, the same channel as the per-skin "opacity" setting; window-level WS_EX_LAYERED opacity would break WebView2 compositing and is not used). Three landing surfaces: entrance at creation is **baked/injected** (local skins = the pure-CSS `deskFadeIn` keyframes in the bridge CSS — no JS dependency; web skins = an `initialization_script`); runtime visibility = `factory::show/hide_skin_window_fade` (eval fade-in to the current effective opacity after show; fade-out then land hide after a delay, guarded by the `FADE_GENS` per-label generation counter — never reset, monotonically increasing, and bumped at **two** anchors: the fade-out start in fade_out_for_destroy **and** the destroy point in close_skin_window_nowait — dropping either reopens the "re-armed during the fade wait" hole); destruction = `fade_out_for_destroy` blocks until the fade finishes before `destroy_skin_window` (the exit path does not fade). Every visibility path (manager button / tray check / hotkey / Alt+F4 / skin menu "Hide skin" / skin_hide·skin_show / focus-mode hide action / layout visibility restore) goes through these two helpers; with the system's "reduce motion" (prefers-reduced-motion) on, all animations are skipped and values settle instantly. Duration anchors = `skin::types::FADE_IN_MS/FADE_OUT_MS` (shared by all four surfaces — never scatter literals). See "Skin Entrance/Exit (Fade In/Out)" in critical-mechanisms.
...
- **Full backend tests**: `cargo test --manifest-path src-tauri/Cargo.toml` (mandatory after changes; currently 165 items — the policy table's three completeness tests, the window-broadcast completeness scan, snapping regressions, package safety, backup rollback, update verification, …; hardware probes are ignored and run manually).
