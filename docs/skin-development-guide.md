...
> **Entry name**: the examples here use the recommended entry `window.driftlet`; `window.__DESK_PP__` found in older skins is the same object under its legacy name, kept forever — no changes needed. An optional wrapper `demos/driftlet.js` (named functions like `Driftlet.getCpuInfo()`) plus `demos/driftlet.d.ts` (editor autocomplete) can be copied into your skin folder.
...
- Right-click anywhere on the page is taken over by the app by default (it shows a native menu: "Open Skin Settings / Reload Skin", then a window-behavior quick-toggle section (On top / Disable dragging / Click-through / Resize by dragging / Edge snapping — checkmark = current state), then "Hide Skin / Unload Skin"; WebView2's default menu is disabled). If your skin needs to handle right-click on a specific element (e.g. right-click-to-edit on a card): listen for `contextmenu` and call `preventDefault()` — the bridge checks `defaultPrevented` at the end of the window bubble path, and clicks the skin consumed will not open the host menu. (Hosts on 1.0.1 or earlier lack this contract; fall back to a window capture-phase listener plus `stopPropagation()`.)
- **You can add custom items to that right-click menu** (skin-side `skin_set_menu_items`, no permission needed): after the fixed items your entries appear with a separator (`checked` checkmark state supported), and a selection is delivered back as the `desk-skin-menu-item` DOM event (`e.detail.id`). Rules: ≤8 items; `id` lowercase letters/digits/hyphens/underscores ≤32 and unique within the list; `label_zh`/`label_en` (or single-language `label`) ≤40 chars, at least one non-empty; pass an empty array to clear. Items are removed on unload — re-register after every reload. Reference: `examples/isles-countdown` (right-click quick size-tier switch).
...
| `gpu_adapter` | GPU adapter picker | `"0x0001A2B3_0x0000F0E1"` | The manager enumerates the machine's GPUs at render time to build the dropdown (model name + integrated/discrete tag); value = LUID string (the stable identity — enumeration order/index can shift and names can repeat), empty = first (auto); skins match exactly via the `luid` field on `get_gpu_info` entries, falling back to the first adapter on mismatch; when the stored value no longer matches (adapter removed/disabled, or a driver update changed its LUID) the dropdown keeps the row labeled "Unavailable adapter (LUID)" — it disappears once you pick a valid one |
...
Not listening is harmless — the bridge bakes the new language on the next reload anyway. See `demos/controls-demo` (§10) for a complete reference implementation.
...
**Command cheat sheet** (full contracts in §5.2–§5.4; the optional wrapper `demos/driftlet.js` turns commands into named functions like `Driftlet.getCpuInfo()`, with `driftlet.d.ts` for editor autocomplete):
...
| `skin_set_menu_items` | — | Register custom items on your own right-click menu (≤8, checked state, `desk-skin-menu-item` event, §3.2) |
...
```
(`path` is a percent-encoded absolute path; driftlet.js's `fileUrl(path)` builds it for you. Skins without `file_system` always get 404 from this endpoint; UNC paths (`\\host\share`) are always rejected — probing them would send NTLM auth outward. **Web skins, note**: when a skin whose `entry` is an http(s) URL declares `file_system`, the remote site cannot call any command (remote origins are rejected by the framework), so `__fs__` is its only reachable capability and it lands on the site's current content — an `http://` entry's page can additionally be rewritten by a network middleman; use this only on https sites you fully control.)
...
- with the `demos/driftlet.js` wrapper, a missing bridge makes every command reject a clear `Error` — catch it and the page renders fine offline.
