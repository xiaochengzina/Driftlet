...
- **新增六张屿族官方皮肤**：倒数日、日历、时钟、系统占用、天气、计时器——同一纸卡设计语言、四套主题预设一键换肤；随安装包内置，干净安装后皮肤库即见「默认皮肤」组（想用哪张点加载即可；这是一次性种子——你删过的皮肤不会复活，自己更新过的同名皮肤也不会被覆盖）。
- **皮肤可在 skin.json 声明边缘吸附默认值（`window.edge_snap` / `window.snap_gap`）**：创作者可让皮肤出厂即带边缘吸附与吸附间距（≤200px，加载时归一化）；屿族六张官方皮肤已全部声明开启（20px 间距）——平台默认仍关，第三方皮肤不受影响；**已持久化过窗口配置的皮肤也跟随 manifest 新声明**（除非你在「窗口」页显式改过吸附——显式选择永远优先）。pack-skin 已镜像重建。
- **分组 ⋯ 菜单新增「删除分组和所有皮肤」**：与「删除分组（皮肤回落未分组）」并列的不可逆档——**两段式二次确认**（明细页 →「最后确认」页才执行），成员皮肤逐个先卸载再删除（文件夹与设置一并消失）；部分失败时组照删、失败皮肤回落「未分组」并单独报错计数。
- **皮肤右键菜单新增窗口行为快捷开关段**：右键皮肤窗口，「刷新皮肤」与「隐藏皮肤」之间新增五个勾选开关——置顶 / 禁止拖动 / 鼠标穿透 / 拖拽调整大小 / 边缘吸附，勾选态 = 当前态；与编辑器「窗口」页同一实现同一事件通道——面板改动菜单跟随、菜单点选面板即时翻转，不用打开管理器就能调窗口行为。
- **皮肤右键菜单自定义项（创作者接口）**：皮肤可给自己的右键菜单注册自定义项（新命令 `skin_set_menu_items`，免权限、≤8 条、支持勾选态与双语标签）——固定四项之下以分隔线成段展示，点选经 `desk-skin-menu-item` 事件回投皮肤。官方倒数日皮肤已落地首个用法：右键即可在「横条 2×1 / 小方 1×1」间快速切换（勾选态 = 当前档，与设置面板同一设置项联动）。`demos/driftlet.js` 可选封装已补 `setMenuItems(items)` / `onMenuItem(fn)`。
- **新增 GPU 适配器选择控件（`gpu_adapter`，控件 22 → 23 种）**：皮肤设置面板运行时枚举本机 GPU 生成下拉（型号名 + 核显/独显标签），值存 LUID——名称可重复、枚举顺序会变，LUID 才是稳定标识（独显拔出/禁用后旧值保留显示不跳变）；空串 = 首项（自动）。皮肤经 `get_gpu_info` 返回项新增的 `luid` 字段精确匹配，失配回退首项。pack-skin 已镜像重建。
- **更新下载国内加速（多通道竞速 + 分段并行）**：自动下载新版安装包时，GitHub 直连与加速镜像通道同时开工、谁先完成用谁，单通道内再拆 8 段并行下载——此前部分地区直连 GitHub 只有十几 KB/s、经常超时失败，只能手动去网页下载；现在下载成功率与速度都大幅提升。安全不妥协：安装包官方 SHA-256 随发布信息一并校验（发布说明附官方哈希时镜像通道才启用），落盘哈希不符立即作废；全部分通道失败才回退「前往下载」。
- **更新弹窗即时弹出 + 下载进度条**：发现新版本立即弹出提示（不再等安装包下完才弹），下载期间显示进度条与当前所用通道；下载完成主钮自动转为「立即安装」，全部通道失败转为「前往下载」。
- **已下载安装包免重下**：同一版本的安装包下载完成、或上次点了「稍后」后，重启应用不再重复下载，直接提示「立即安装」。

### 变更

- **屿族六张皮肤出厂默认配色统一为澄空主题**（bg/text/accent 全部对齐澄空值——此前系统占用默认青柠、计时器/倒数日默认 coral 环/数字、时钟板岩灰蓝秒针造成开箱全家福不统一，需手动调主题）；系统占用澄空主题中环由偏绿的青改为天青（蓝→天青→紫同冷族，蓝纸上不再割裂）、极光主题三环改为极光绿→青→长春蓝（原薄荷+浅蓝+粉在深底上读成糖果三拼）。
- **天气皮肤排版打磨**：当天温度区间改为**低到高**（与通行天气应用一致，此前高~低反直觉）；未设置城市时整卡显示引导（中央云图标 + 说明，横条/小方同款）；**数据加载期间同样整卡提示「正在加载天气数据」**——半成品帧（--° + 空图标）不再闪现，取数失败且无缓存可回落时保持加载态下个节拍再试；右区未来五日预报整列撑满高度（星期/图标/温度区间/日期分推）；负两位数温度（-10°）排版不溢出；浅色主题下天气图标增加随主色走的投影描边，白云/浅蓝图标在 pastel 卡面上清晰可读。
- **屿族皮肤族视觉统一 + 主题系统收敛**：主题预设从十一套收为四套（澄空/青柠/薰衣/极光暗）并打磨配色；移除「自定义」项（任意主题都能再经调色板微调，纯属多余）——六张皮肤出厂同一纸卡语言。**倒数日**：场景颜色预设整体移除（自定义背景图保留，暗纱/模糊只为图片服务），新增专属「图片背景」主题预设（白字 + 黑暗纱，配合选图用）；横条布局修复窗口拉伸时内容漂移——左列眉题顶对齐、右列大数字沉底。**族共享基座**：多层渐变「场景类」组件随之从 base.css 移除（族皮肤已全部纸卡化，仅自定义图片卡的 photo 机制保留）。
- **系统占用皮肤 GPU 自动档改为独显优先**：「GPU 数据源 = 自动」时从此前取枚举首项（双显机器上常是核显，占用曲线没有参考价值）改为优先取独显，无独显才回落枚举首项；指定适配器失配（拔出/禁用/驱动更新换 LUID）的回退口径同步为独显优先。管理器下拉顶项文案同步为「自动（独显优先）」。
- **新建/编辑分组对话框的皮肤清单：未分组皮肤靠前**——先挑未分组的入组更顺手；其余皮肤保持列表原顺序，勾选态与「现属组」小签不随排序错位。

### 修复

- **窗口层级切换与分组保存失败时正确回滚**：皮肤「置顶↔贴桌面」原位翻转的窗口操作失败时，配置回滚到翻转前并落盘（此前配置说已翻转、窗口没动）；分组保存失败时内存态回滚（此前内存已改、盘上照旧、下一次任意保存会带出分歧）。
- **首装内置皮肤拷贝原子化**：安装包内置皮肤的首装拷贝改为暂存目录 + 原子换入，中途失败/崩溃不再留下被 `dest.exists()` 永久挡住补拷的半成品目录。
- **安全：`__fs__` 端点拒绝 UNC 路径**：声明 `file_system` 的皮肤此前能用 `\\主机\共享` 形式路径让宿主在文件探测时向外部 SMB 服务器发起 NTLM 认证（认证哈希外泄面）；现与命令通道同口径一律拒绝（权限声明语义本就是「本机」任意路径）。
- **皮肤文件自写改原子写**：皮肤写自己目录文件（`skin_write_file`——计时器状态、天气缓存、自定义背景图都走这里）改为临时文件 + 重命名，崩溃中写不再留下损坏的半个文件。
- **皮肤主题预设切换末项颜色滞留修复**：六张屿族皮肤此前切主题时偶发最后一个颜色项滞后一拍（皮肤侧写设置的回同步是异步的）——现主题配色以本地预设值先行生效，不再等回同步。
- **天气皮肤**：拼错城市时整卡明示「找不到城市」（不再永远停在加载中）；跨零点立即重拉（「今天/明天」标签不滞留）；英文界面城市名显示英文；快速连改城市时晚到的旧请求不再回写旧帧。
- **计时器/倒数日键盘可达性**：hover 才浮现的按钮（开始/暂停/重置、环内步进钮、换背景图钮）此前因 visibility 隐藏被移出 Tab 键序，键盘完全够不到——现 Tab 聚焦即显形。
- **倒数日**：「图片背景」主题预设但未选图时文字回落安全深色（不再白字白底）；自定义背景删除失败时不再静默切回纸卡（文件还在、下回会复活——失败记日志并保持原状）。
- **计时器不再空转**：200ms 界面节拍只在「计时运行中 + 页面可见」时存在——空闲/暂停/完成/隐藏时零轮询（此前恒转）；隐藏期间倒计时到点，系统通知与提示音仍准点弹出（一次性到点兜底，不等唤回），唤回即续拍。
- **GPU 适配器控件失效项不再显示裸 LUID**：存值对应的适配器消失时（独显拔出/禁用，或驱动更新给同一显卡换了 LUID——实机反馈），下拉此前会把原始 LUID 当成一行选项（`0x00000000_0x0000EC46` 这类无意义串）。现显示「已失效的适配器（LUID）」明示文案，换选任意有效项即消失；皮肤侧失配回退首项的行为不变。
- **卸载不再残留更新目录**：此前卸载只清理 `config/` 与 `skins/`，自动更新下载到 `update/` 目录的安装包会残留，导致整个安装目录删不掉。现卸载（非更新模式）直接清空安装目录的全部内容——请注意：卸载会删除安装目录内的所有文件与文件夹，请勿在其中存放自己的文件。

## [1.2.2] - 2026-08-29

### 变更

- **预览图内存瘦身（管理器更省内存）**：「截取预览」落盘前自动降采样——最长边压到 640px（管理器列表显示区只有 226×112，全尺寸截图解码后每张 ~8MB 位图常驻内存），体积同步大幅缩小；降采样失败自动回退原图，不影响截图功能本身。创作者自带的 `preview.png/jpg/jpeg` 新增尺寸校验：最长边 ≤1280px（安装与打包时检查，超限给出明确报错）——超大预览在列表里只会拖慢解码、白占内存。

- **单语言皮肤文案回退规则 + 文案字段对称更名（单语创作者零负担）**：此前 `*_en` 字段只有声明 `bilingual: true` 才生效——只做英文皮肤的创作者不声明多语言时，英文名在英文界面下也会被忽略。现在改为「界面语言优先、缺失回退另一语言」：只填一种语言的单语言皮肤在中、英界面下都显示创作者提供的那种语言；任何字段组合都合法，**`bilingual` 开关已移除**（不存在「填了被忽略」的陷阱）。同时文案字段对称更名——`name`/`description`/`label`/`group` → `name_zh`/`description_zh`/`label_zh`/`group_zh`，与 `*_en` 成对；**旧字段名继续被接受**（解析按等价别名读取，存量皮肤零迁移；副本/从源同步改写 manifest 时自动迁入新键名）。皮肤列表/编辑器/安装向导/备份审查/托盘菜单同一规则。

### 修复

- **未加载皮肤的显隐快捷键钮禁用**（编辑器「窗口」页）：皮肤专属显隐热键注册表常驻、未加载时按下本就无效果，但录制钮此前未随加载态禁用——给没运行的皮肤配热键没有即配即验的反馈，易配完即忘。现与紧邻的置顶/锁定/穿透控件一致：未加载即禁用，加载后恢复可点。
