...
- **Manager commands are callable only from the manager window**: all management commands (load / unload / settings, etc.) verify the caller window's identity; calls from skin windows are always rejected (with only a few harmless self-action exceptions such as dragging and frame resizing — see `policy.rs` for the authoritative list). Every IPC command is registered with its caller tier in the policy table at `src-tauri/src/policy.rs`, and a completeness test enforces "a new command must be registered and carry the matching gate in its body" — a missing gate fails the build instead of relying on human review.
