...
- **管理器命令仅管理器窗口可调**：加载/卸载/设置等全部管理命令校验调用方窗口身份，皮肤窗口调用一律拒绝（仅拖动/边框缩放等少数无害自作用命令例外——逐条枚举以 `policy.rs` 为准）。全部 IPC 命令在 `src-tauri/src/policy.rs` 的策略表逐条登记调用方档位，完备性测试强制「新命令必须登记且函数体带对应闸门」——漏设防即测试失败，不靠人肉核对。
