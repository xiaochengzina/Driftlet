...
- 皮肤权限模型：敏感能力需在 `skin.json` 声明 `permissions`（注册表 / Shell / 系统控制（打开外部/通知/电源与回收站）/ 剪贴板 / 麦克风 / 任意路径文件读写 / 皮肤控制 / 媒体控制共 8 种），安装引导页逐条展示并按高危/中危两档分级标注（见「安全模型」）
...
- **无需管理员权限**：全部能力在标准用户下可用（程序清单 asInvoker）；若被已提权的终端/启动器拉起，启动时自动经一次性任务计划以普通权限重新拉起自己并退出提权进程；降权失败则提示并退出，拒绝提权运行（`DRIFTLET_ALLOW_ELEVATED=1` 可显式放行；debug 构建默认不降级）。

- **权限声明**：注册表、Shell、系统控制（打开外部/通知/锁屏/睡眠/关机/清空回收站）、剪贴板、麦克风、任意路径文件读写（`file_system`）、皮肤窗口配置与生命周期控制（`control`）、媒体控制（`media`）共 8 种敏感能力，皮肤必须在 `skin.json` 声明 `permissions` 才可调用，后端逐命令强制校验；安装引导页逐条展示声明并按两档分级标注（高危 `shell` / `system` / `file_system` 标红，中危 `registry` / `clipboard` / `mic` / `control` / `media` 标黄）。
