...
- 皮肤权限模型：敏感能力需在 `skin.json` 声明 `permissions`（注册表 / Shell / 系统控制（打开链接/电源与回收站）/ 剪贴板 / 麦克风 / 任意路径文件读写 / 皮肤窗口控制 / 媒体控制 / 系统通知 / 系统信息 / 网络请求 / 打开链接共 12 种），安装引导页逐条展示并按高危/中危/低危三档分级标注（见「安全模型」）

| 安装引导页 |
|:---:|
| ![安装引导页](docs/screenshots/zh-07-install-wizard.png) |

- 皮肤自定义设置：`skin.json` 声明配置项（23 种控件 + 分组 + 描述），配置面板自动生成
...
| 皮肤设置 | 专注模式 |
|:---:|:---:|
| ![编辑器·皮肤设置页签](docs/screenshots/zh-03-editor-skin-settings.png) | ![专注模式面板](docs/screenshots/zh-04-focus-mode.png) |
...
- 布局方案：把当前桌面状态（加载集 + 各皮肤位置/尺寸/显隐）存为命名方案，一键应用切换，托盘菜单同步列出可应用

| 设置 | 布局方案 |
|:---:|:---:|
| ![设置页](docs/screenshots/zh-05-settings.png) | ![布局方案面板](docs/screenshots/zh-06-layouts.png) |
...
│   ├── toolbox/              # 本机工具箱（剪贴板/文件/注册表/命令/外链/电源/设置读写）
│   ├── deepseek-balance/     # DeepSeek 余额自动查询（联网皮肤参考，预警+系统通知+充值页，password 存 API Key）
│   ├── power-tools/          # 权限能力演示（任意路径文件读写 file_system 高危 / 皮肤窗口配置控制 control 中危）
│   ├── web-view/             # 网页内嵌（iframe 嵌任意站点页面的通用皮肤：本地外壳 + 桥全功能，零权限）
...
可选封装 `demos/driftlet.js` 把命令包成 `Driftlet.getCpuInfo()` 这类命名函数（配 `driftlet.d.ts` 有编辑器补全）；完整命令清单与契约见 `docs/皮肤开发指南.md` 第 5 章。
...
参考示例：`demos/controls-demo`（全部 23 种控件演示，界面语言跟随管理器）。
...
- **无需管理员权限**：全部能力在标准用户下可用（程序清单 asInvoker）；若被已提权的终端/启动器拉起，启动时提醒一次（说明影响：shell 权限皮肤可静默以管理员权限执行命令、Explorer 拖 .dskin 被拦截可改用双击/文件选择器）——「是」= 继续运行以后不再提示，「否」= 退出（`DRIFTLET_ALLOW_ELEVATED=1` 同样放行；debug 构建默认不提醒）。不自动降级：任务计划代起触发安全软件行为检测、令牌 API 依赖管理员特权（标准用户提权拉起时不可用），均已实测移除。

- **权限声明**：注册表、Shell、系统控制（打开链接/锁屏/睡眠/关机/清空回收站）、剪贴板、麦克风、任意路径文件读写（`file_system`）、皮肤窗口配置与生命周期控制（`control`）、媒体读取与控制（`media`）、系统通知（`notify`）、只读系统信息（`sys_info`）、网络请求（`network`）、打开网页链接（`open_link`）共 12 种敏感能力，皮肤必须在 `skin.json` 声明 `permissions` 才可调用，后端逐命令强制校验；安装引导页逐条展示声明并按三档分级标注（高危 `shell` / `system` / `file_system` 标红，中危 `registry` / `clipboard` / `mic` / `control` 标黄，低危 `media` / `notify` / `sys_info` / `network` / `open_link` 标蓝）。打开链接只收 http(s)/mailto/ms-settings URI 白名单——本地路径面整体裁撤（可执行扩展名黑名单是追不上执行面的负枚举），确需打开本地文件的皮肤走 `shell` 权限明示。
- **管理器命令仅管理器窗口可调**：加载/卸载/设置等全部管理命令校验调用方窗口身份，皮肤窗口调用一律拒绝（仅拖动、边框缩放、右键菜单三个无害命令例外）。全部 IPC 命令在 `src-tauri/src/policy.rs` 的策略表逐条登记调用方档位，完备性测试强制「新命令必须登记且函数体带对应闸门」——漏设防即测试失败，不靠人肉核对。
- **皮肤窗口零授权**：皮肤窗口的 capabilities 不授予任何 Tauri 核心/插件权限，只能经注入桥 `__DESK_PP__.invoke` 调后端命令。
- **文件沙箱**：皮肤的文件读写限定在自身目录内（拒绝绝对路径与 `..` 逃逸），`skin.json` / `settings.json` 禁写禁删；声明了 `file_system` 的皮肤也不得变更应用数据根（skins/config）、更新目录与程序目录——防止改写 skin.json 静默提权、或改写宿主会执行的文件（安装包/exe/dll）。
- **更新通道完整性**：安装包自动下载时记录 SHA-256（版本标记内），「立即安装」执行前复核标记完整 + 版本新于当前 + 文件哈希一致，缺一即拒——用户确认的「安装官方更新」不能执行被第三方改写过的安装包。
- **设置值跨皮肤隔离**：`settings.json` 被 skin:// 协议拦截（含 8.3 短名、ADS 等绕过手段），A 皮肤读不到 B 皮肤的设置；`password` 类型值不落页面，由 `skin_get_setting` 按窗口身份下发。
- **`.dskin` 安装防护**：解压防 zip-slip 与 zip 炸弹（按实际解压字节计量），体积/文件数上限 256MB / 1GB / 10000；staging 回滚式安装，失败不破坏旧版本。

---

## Code signing policy（代码签名政策）

本项目曾申请 SignPath Foundation 免费代码签名，但因项目知名度不足（GitHub stars、社区讨论等外部信号不够）于 2026-09-16 被拒绝。**当前发布的安装包未进行代码签名**。
...
- **未来计划**：待项目获得更广泛的社区认可后重新申请 SignPath Foundation；或考虑购买商业代码签名证书（如 Azure Trusted Signing）。
