...
- 皮肤权限模型：敏感能力需在 `skin.json` 声明 `permissions`（文件 / 注册表 / Shell / 系统控制 / 剪贴板 / 麦克风共 6 种），安装引导页逐条展示并标注高危项（见「安全模型」）
- 皮肤自定义设置：`skin.json` 声明配置项（19 种控件 + 分组 + 描述），配置面板自动生成
- 调整皮肤的不透明度、位置、大小、缩放比例（「窗口」页可开拖拽缩放：显示边框提示，拖边缘/四角直接调整；缩放比例 50%–200% 整体缩放窗口与内容）
- 窗口置顶 / 贴在桌面（二选一，默认贴在桌面）、锁定位置
- 为皮肤截取预览图
- 托盘图标管理，主窗口关闭即隐藏到托盘
- 开机自启、暗/亮主题切换
- 右键皮肤窗口打开皮肤菜单（打开配置 / 刷新 / 卸载）
- 全局快捷键一键隐藏/显示已加载的皮肤（默认 Ctrl+Shift+Alt+D，可在设置中修改或禁用），托盘菜单同步勾选项
...
开发构建（`npm run tauri dev`）下皮肤文件保存后自动重载（防抖 300ms），无需手动右键刷新；不需要时可在设置页关闭。
...
参考示例：`examples/controls-demo`（全部 19 种控件演示，界面语言跟随管理器）。
...
- **权限声明**：文件、注册表、Shell、系统控制（音量/媒体/打开外部/通知）、剪贴板、麦克风共 6 种敏感能力，皮肤必须在 `skin.json` 声明 `permissions` 才可调用，后端逐命令强制校验；安装引导页逐条展示声明，`shell` / `mic` 标高危。
...
- "窗口置顶" 和 "贴在桌面" 二选一，始终启用其中一种，默认 "贴在桌面"。
