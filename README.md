...
- 皮肤自定义设置：`skin.json` 声明配置项（22 种控件 + 分组 + 描述），配置面板自动生成
...
- 皮肤分组管理：自定义分组、折叠、改名与勾选式成员编辑；组内批量加载/卸载/隐藏/显示
- 皮肤多开：同一皮肤可创建副本独立运行（独立设置/预览/权限），源皮肤更新后「从源同步」一键追平
- 启动更新检测（默认开，可在设置中关闭）：发现 GitHub 新版本后后台自动下载安装包（固定名 `update/Driftlet-update-setup.exe`，自动覆盖不堆积），下载完成才弹「立即安装」提示；下载失败时降级为「前往下载页面」
...
参考示例：`examples/controls-demo`（全部 22 种控件演示，界面语言跟随管理器）。
