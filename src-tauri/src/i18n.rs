...
fn zh(key: Key) -> &'static str {
    match key {
        ...
        Key::OpenFailed => "打开失败: {}",
        Key::SkinAlreadyInstalled => "皮肤 '{}' 已安装",
        Key::InvalidSkinId => "皮肤 id '{}' 不合法：仅允许小写字母、数字、中划线，且必须以字母或数字开头",
        Key::InvalidFolderName => "文件夹名无效",
        Key::CopySkinFailed => "复制皮肤失败: {}",
        Key::SkinDirNotExist => "皮肤目录不存在",
        Key::MissingSkinJson => "缺少 skin.json",
...
fn en(key: Key) -> &'static str {
    match key {
        ...
        Key::SkinAlreadyInstalled => "Skin '{}' is already installed",
        ...
        Key::InvalidFolderName => "Invalid folder name",
        Key::CopySkinFailed => "Failed to copy skin: {}",
        Key::SkinDirNotExist => "Skin directory does not exist",
        Key::MissingSkinJson => "Missing skin.json",
        ...
        Key::NetworkError => "Network request failed: {}",
