  ...
  match event {
    ...
    Event::UserEvent(message) => match message {
      ...
      Message::Window(id, WindowMessage::Destroy) => {
        // NOTE(driftlet): when the HWND was already destroyed externally
        // (e.g. a wallpaper-layer skin SetParent'ed into Progman and killed
        // together with explorer), tao never fires WindowEvent::Destroyed —
        // its Window::drop only PostMessages the dead handle — so the entry
        // would leak in this store AND in tauri's manager maps (label stuck
        // forever, recreate fails with "already exists"). Detect the dead
        // handle up front and run the full Destroyed flow ourselves:
        // forward the event (lets tauri clean its manager maps), remove the
        // entry, then apply the same empty-store exit logic as the
        // TaoWindowEvent::Destroyed arm above.
        let label = if hwnd_is_dead(&id, &windows) {
          windows.0.borrow().get(&id).map(|w| w.label.clone())
        } else {
          None
        };
        ...
        if let Some(label) = label {
          ...
          let removed = windows.0.borrow_mut().remove(&id).is_some();
          if removed && windows.0.borrow().is_empty() {
            let (tx, rx) = channel();
            callback(RunEvent::ExitRequested { code: None, tx });
            let recv = rx.try_recv();
            let should_prevent = matches!(recv, Ok(ExitRequestedEventAction::Prevent));
...
// NOTE(driftlet): true when the window's HWND no longer exists at OS level
// (destroyed externally, e.g. a child of a killed Progman). The
// missing-Destroyed-event problem this supports is Windows-specific; other
// platforms always report false.
#[cfg(windows)]
fn hwnd_is_dead(window_id: &WindowId, windows: &Arc<WindowsStore>) -> bool {
  ...
  let hwnd = windows
    ...
    .borrow()
    .get(window_id)
    .and_then(|w| w.inner.as_ref().map(|inner| inner.hwnd()));
  match hwnd {
    Some(hwnd) => !unsafe { IsWindow(Some(HWND(hwnd as *mut _))) }.as_bool(),
    None => false,
  }
}

#[cfg(not(windows))]
fn hwnd_is_dead(_window_id: &WindowId, _windows: &Arc<WindowsStore>) -> bool {
  false
}
